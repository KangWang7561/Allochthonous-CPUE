...
discr = load_discr_data(filename = "TEF.csv",
                        mix)

output_options = list(summary_save = TRUE,                 
                      summary_name = "summary_Lake+Species", 
                      sup_post = TRUE,        
                      ...
                      geweke = TRUE,   
                      diag_save = TRUE,
                      diag_name = "diagnostics_Lake+Species",
                      indiv_effect = FALSE,       
...
saveRDS(jags.mod, file = paste0("MixSIAR_Lake+Species", ".rds"))
saveRDS(mix, file = paste0("mix_Lake+Species",".rds"))
saveRDS(source, file = paste0("source_Lake+Species",".rds"))
saveRDS(discr, file = paste0("discr_Lake+Species",".rds"))

# Posterior ####
mix = readRDS(file = paste0("mix_Lake+Species",".rds"))
source = readRDS(file = paste0("source_Lake+Species",".rds"))
jags = readRDS(file = paste0("MixSIAR_Lake+Species",".rds"))
...
gg = cbind(rep(rep(mix[["data"]][["Lake"]], each = number.sim), times = number.sources),  
           rep(rep(mix[["data"]][["Species"]], each = number.sim), times = number.sources))
...
colnames(Posterior) = c("Draw", "Proportion", "Source", "Lake", "Species")
...
Posterior.Species = Posterior %>%
  group_by(Lake, Species, Source) %>%  
  summarise(y0.025 = quantile(Proportion, 0.025, na.rm = TRUE),
            y0.1= quantile(Proportion, 0.1, na.rm = TRUE),
            y0.25= quantile(Proportion, 0.25, na.rm = TRUE),
            y0.375= quantile(Proportion, 0.375, na.rm = TRUE),
...
# write_xlsx(Posterior.Species, path = "Posterior.Species.xlsx")
